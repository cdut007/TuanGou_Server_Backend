...
class ConsumerOrderView(APIView):
    ...
    def delete(self, request):
        # 删除订单
        order_id = request.data['order_id'] if request.data.has_key('order_id') else request.GET['order_id']
        order_goods = GenericOrder.objects.get(pk=order_id)
        if not self.delete.user_id == order_goods.user_id:
            return Response(format_body(22, 'Fail', 'xiajibagao'))
        order_goods.delete()
        ...
        if int(order_goods.goods.group_buy.award_red_packets):
            ...
            if not remain_order:
                UnpackRedPacketsLog.objects.filter(
                    ...
                    group_buying_id = order_goods.goods.group_buy_id
                ).update(is_failure = 2)
